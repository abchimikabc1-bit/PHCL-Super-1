...
import {
  MAX_ORDER_STATUS_AUDIT,
  appendOrderStatusAudit,
  getCustomerOverrideMap,
  getDeliveredAtMap,
  getOrderStatusAudit,
  getOrderStatusMap,
  saveCustomerOverrideMap,
  saveDeliveredAtMap,
  saveOrderStatusMap,
...
import { WalletCurrency, creditWalletBalance, debitWalletBalance } from '@/lib/wallet-storage';
...
export default function AdminOrdersPage() {
  ...
  const [reconciliationFilter, setReconciliationFilter] = useState<'all' | 'missing_payment_ref' | 'missing_refund_ref' | 'fully_reconciled'>('all');
  ...
  useEffect(() => {
    ...
    setOrderStatusMap(snapshot.orderStatusMap);
    setStatusAuditEvents(snapshot.orderStatusAudit);
    setCustomerOverrideMap(snapshot.customerOverrideMap);
    setDeliveredAtMap(snapshot.deliveredAtMap);
  }, [snapshot]);
  ...
  useEffect(() => {
    ...
    if (isAuthenticated) {
      void refreshCommerceClientCache().finally(syncOrders);
  ...
  const reconciliationSummary = useMemo(() => {
    let ordersWithPaymentTxn = 0;
    let cancelledOrders = 0;
    let cancelledWithRefundTxn = 0;
    let missingPaymentTxn = 0;
    let missingRefundTxn = 0;

    const missingPaymentOrderIds: string[] = [];
    const missingRefundOrderIds: string[] = [];

    for (const order of summary.mergedOrders) {
      if (order.audit?.paymentTransactionId) {
        ordersWithPaymentTxn += 1;
      } else {
        missingPaymentTxn += 1;
        if (missingPaymentOrderIds.length < 5) {
          missingPaymentOrderIds.push(order.id);
        }
      }
      ...
      if (order.audit?.cancellation?.cancelledAt) {
        ...
        if (order.audit.cancellation.refundTransactionId) {
          cancelledWithRefundTxn += 1;
        } else {
          missingRefundTxn += 1;
          if (missingRefundOrderIds.length < 5) {
            missingRefundOrderIds.push(order.id);
          }
        }
      }
    }

    const paymentCoveragePct = summary.mergedOrders.length > 0
      ? (ordersWithPaymentTxn / summary.mergedOrders.length) * 100
      : 100;
    const refundCoveragePct = cancelledOrders > 0
      ? (cancelledWithRefundTxn / cancelledOrders) * 100
      : 100;

    return {
      totalOrders: summary.mergedOrders.length,
      ordersWithPaymentTxn,
      cancelledOrders,
      cancelledWithRefundTxn,
      missingPaymentTxn,
      missingRefundTxn,
      missingPaymentOrderIds,
      missingRefundOrderIds,
      paymentCoveragePct,
      refundCoveragePct,
    };
  ...
  const filteredOrders = useMemo(() => {
    ...
    return summary.mergedOrders.filter((order) => {
      ...
      const hasPaymentTxn = Boolean(order.audit?.paymentTransactionId);
      const isCancelled = Boolean(order.audit?.cancellation?.cancelledAt);
      const hasRefundTxn = Boolean(order.audit?.cancellation?.refundTransactionId);
      ...
      const matchesReconciliation =
        ...
        (reconciliationFilter === 'missing_payment_ref' && !hasPaymentTxn) ||
        (reconciliationFilter === 'missing_refund_ref' && isCancelled && !hasRefundTxn) ||
        (reconciliationFilter === 'fully_reconciled' && hasPaymentTxn && (!isCancelled || hasRefundTxn));
      const matchesSearch =
        term.length === 0 ||
      ...
      return matchesMethod && matchesStatus && matchesConsent && matchesReconciliation && matchesSearch;
    });
  }, [summary.mergedOrders, searchTerm, methodFilter, statusFilter, consentFilter, reconciliationFilter, complianceSummary.byOrderId, orderStatusMap]);
  ...
  const executeOrderCancellation = (
    ...
    actor: string,
    reason?: string
  ): { success: boolean; alreadyCancelled?: boolean; message?: string } => {
    const liveOrder = getOrderIntegrityReport().validOrders.find((order) => order.id === orderId);
    if (!liveOrder) {
      return { success: false, message: `Order ${orderId} not found` };
    }

    if (liveOrder.audit?.cancellation?.refunded || liveOrder.audit?.cancellation?.cancelledAt) {
      return { success: true, alreadyCancelled: true };
    }

    if (!liveOrder.items || liveOrder.items.length === 0) {
      return { success: false, message: `${orderId} has no item details for stock restore` };
    }

    const refundCurrency = liveOrder.paymentMethod as WalletCurrency;
    const refundAmount = convertAmount(liveOrder.totalUsd, 'usd', refundCurrency);

    const stockRestore = revertProductStockPurchase(
      liveOrder.items.map((item) => ({ productId: item.id, quantity: item.quantity })),
      `admin_refund:${orderId}`
    );

    if (!stockRestore.success) {
      return { success: false, message: stockRestore.reason || `Could not restore stock for ${orderId}` };
    }

    const walletRefund = creditWalletBalance(refundCurrency, refundAmount, 'admin_order_refund', orderId);
    if (!walletRefund.success) {
      applyProductStockPurchase(
        liveOrder.items.map((item) => ({ productId: item.id, quantity: item.quantity })),
        `admin_refund_rollback:${orderId}`
      );
      return { success: false, message: `Could not refund wallet for ${orderId}` };
    }

    const updated = updateOrderById(orderId, (current) => {
      if (current.audit?.cancellation?.refunded || current.audit?.cancellation?.cancelledAt) {
        return current;
      }
    ...
    if (!updated) {
      debitWalletBalance(refundCurrency, refundAmount, 'admin_refund_rollback', orderId);
      applyProductStockPurchase(
        liveOrder.items.map((item) => ({ productId: item.id, quantity: item.quantity })),
        `admin_refund_rollback:${orderId}`
      );
      return { success: false, message: `Could not persist cancellation for ${orderId}` };
    }
  ...
  const runBulkStatusAction = (action: 'mark_processing' | 'mark_shipped' | 'mark_delivered' | 'mark_cancelled') => {
    ...
    for (const id of selectedOrderIds) {
      ...
      if (nextStatus === 'cancelled') {
        const cancellation = executeOrderCancellation(id, 'PHCL Administrator', cancelReason);
        if (!cancellation.success) {
          cancelFailed += 1;
          if (cancellation.message) {
            toast.error(cancellation.message);
          }
          continue;
        }
    ...
    if (nextStatus === 'cancelled') {
      if (cancelSucceeded > 0) {
        toast.success(`Cancelled and refunded ${cancelSucceeded} order(s)`);
      }
      if (cancelSkipped > 0) {
        toast.info(`${cancelSkipped} order(s) were already cancelled`);
      }
      if (cancelFailed > 0) {
        toast.error(`${cancelFailed} order cancellation(s) failed`);
      }
    }
  ...
  const exportCsv = () => {
    ...
    const rows = filteredOrders.map((order) => {
      ...
      return [
        ...
        escapeCsv(order.audit?.paymentTransactionId),
        ...
        escapeCsv(order.audit?.cancellation?.cancelledAt),
        escapeCsv(order.audit?.cancellation?.reason),
        escapeCsv(order.audit?.cancellation?.refundCurrency),
        escapeCsv(order.audit?.cancellation?.refundAmount),
        escapeCsv(order.audit?.cancellation?.refundTransactionId),
        escapeCsv(order.audit?.cancellation?.refundedAt),
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        ...
        <div className="mb-6 grid grid-cols-1 gap-3 md:grid-cols-6">
          ...
          <select
            ...
            onChange={(e) => setReconciliationFilter(e.target.value as 'all' | 'missing_payment_ref' | 'missing_refund_ref' | 'fully_reconciled')}
            className="rounded-lg border border-white/20 bg-white/10 px-3 py-2 text-sm text-white focus:outline-none focus:ring-2 focus:ring-purple-500"
          >
            <option value="all">All Reconciliation</option>
            <option value="missing_payment_ref">Missing Payment Ref</option>
            <option value="missing_refund_ref">Missing Refund Ref</option>
        ...
        <div className={`mb-8 rounded-lg border p-4 ${reconciliationSummary.missingPaymentTxn > 0 || reconciliationSummary.missingRefundTxn > 0 ? 'border-amber-300/30 bg-amber-500/10' : 'border-emerald-300/30 bg-emerald-500/10'}`}>
          <h2 className="text-sm font-semibold text-white">Wallet Reconciliation Snapshot</h2>
          <div className="mt-3 grid grid-cols-1 gap-2 text-xs sm:grid-cols-4">
            <span className="rounded bg-slate-900/40 px-2 py-1 text-slate-200">Payment Txn Coverage: {reconciliationSummary.paymentCoveragePct.toFixed(1)}%</span>
            <span className="rounded bg-cyan-500/20 px-2 py-1 text-cyan-300">Orders with Payment Txn: {reconciliationSummary.ordersWithPaymentTxn}/{reconciliationSummary.totalOrders}</span>
            <span className="rounded bg-rose-500/20 px-2 py-1 text-rose-300">Missing Payment Txn: {reconciliationSummary.missingPaymentTxn}</span>
            <span className="rounded bg-violet-500/20 px-2 py-1 text-violet-300">Refund Txn Coverage: {reconciliationSummary.refundCoveragePct.toFixed(1)}%</span>
          </div>
          <div className="mt-3 grid grid-cols-1 gap-2 text-xs sm:grid-cols-3">
            <span className="rounded bg-slate-900/40 px-2 py-1 text-slate-200">Cancelled Orders: {reconciliationSummary.cancelledOrders}</span>
            <span className="rounded bg-emerald-500/20 px-2 py-1 text-emerald-300">Cancelled with Refund Txn: {reconciliationSummary.cancelledWithRefundTxn}</span>
            <span className="rounded bg-amber-500/20 px-2 py-1 text-amber-300">Missing Refund Txn: {reconciliationSummary.missingRefundTxn}</span>
          </div>
          {(reconciliationSummary.missingPaymentOrderIds.length > 0 || reconciliationSummary.missingRefundOrderIds.length > 0) && (
            <div className="mt-3 space-y-1 text-[11px] text-amber-100">
              {reconciliationSummary.missingPaymentOrderIds.length > 0 && (
                <p>Missing payment refs: {reconciliationSummary.missingPaymentOrderIds.join(', ')}</p>
              )}
              {reconciliationSummary.missingRefundOrderIds.length > 0 && (
                <p>Missing refund refs: {reconciliationSummary.missingRefundOrderIds.join(', ')}</p>
              )}
            </div>
          )}
        </div>
                    ...
                    <tr key={order.id} className="hover:bg-white/5">
                      ...
                      <td className="px-4 py-3">
                        <div className="space-y-1 text-xs">
                          ...
                          <p className="text-slate-400">Payment Txn: {order.audit?.paymentTransactionId || 'N/A'}</p>
                          <p className="text-slate-400">Reorder From: {order.audit?.reorderSourceOrderId || 'Direct'}</p>
                          {order.audit?.cancellation?.cancelledAt && (
                            <>
                              <p className="text-rose-200">Cancelled: {new Date(order.audit.cancellation.cancelledAt).toLocaleString()}</p>
                              <p className="text-rose-200">
                                Refund: {order.audit.cancellation.refundCurrency && typeof order.audit.cancellation.refundAmount === 'number'
                                  ? formatCurrencyAmount(order.audit.cancellation.refundCurrency, order.audit.cancellation.refundAmount)
                                  : 'N/A'}
                              </p>
                              <p className="text-rose-200">Refund Txn: {order.audit.cancellation.refundTransactionId || 'N/A'}</p>
                            </>
                          )}
                        </div>
                      </td>
                          ...
                          {status !== 'cancelled' && status !== 'delivered' && (
                            <button
                              ...
                              onClick={() => {
                                ...
                                const cancellation = executeOrderCancellation(order.id, 'PHCL Administrator', reason);
                                if (!cancellation.success) {
                                  toast.error(cancellation.message || 'Unable to cancel order');
                                  return;
                                }
                                if (cancellation.alreadyCancelled) {
                                ...
                                updateOrderStatus(order.id, 'cancelled', 'PHCL Administrator', reason);
                              }}
          ...
          return (
            <div className="fixed inset-0 z-50 flex items-end justify-center bg-black/60 p-4 sm:items-center">
              <div className="max-h-[90vh] w-full max-w-3xl overflow-auto rounded-xl border border-white/10 bg-slate-900 p-5 text-slate-200">
                ...
                {detailOrder.audit?.cancellation?.cancelledAt && (
                  <div className="mt-3 rounded border border-rose-300/20 bg-rose-500/10 p-3 text-xs text-rose-100">
                    <p>Cancelled At: {new Date(detailOrder.audit.cancellation.cancelledAt).toLocaleString()}</p>
                    <p>
                      Refund: {detailOrder.audit.cancellation.refundCurrency && typeof detailOrder.audit.cancellation.refundAmount === 'number'
                        ? formatCurrencyAmount(detailOrder.audit.cancellation.refundCurrency, detailOrder.audit.cancellation.refundAmount)
                        : 'N/A'}
                    </p>
                    <p>Payment Txn: {detailOrder.audit?.paymentTransactionId || 'N/A'}</p>
                    <p>Refund Txn: {detailOrder.audit.cancellation.refundTransactionId || 'N/A'}</p>
                    <p>Refunded At: {detailOrder.audit.cancellation.refundedAt ? new Date(detailOrder.audit.cancellation.refundedAt).toLocaleString() : 'N/A'}</p>
                    {detailOrder.audit.cancellation.reason && <p>Reason: {detailOrder.audit.cancellation.reason}</p>}
                  </div>
                )}
